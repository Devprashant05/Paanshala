...
import {
  Mail, Phone, MapPin, Calendar, Users, CheckCircle,
  Clock, Loader2, Search, X, MessageSquare, PartyPopper,
  InboxIcon, Building2, UtensilsCrossed,
} from "lucide-react";
...
export default function AdminContactsPage() {
  const { contacts, loading, fetchAllContactsAdmin, markContactAsRead } = useContactStore();
  ...
  useEffect(() => { fetchAllContactsAdmin(); }, []);
    ...
    <div className="space-y-7 max-w-5xl">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#2d5016] mb-1">Admin Panel</p>
            <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">Contacts</h1>
            <p className="text-sm text-gray-400 mt-1">Messages and event booking requests</p>
          </div>
          {unreadCount > 0 && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 px-4 py-2 rounded-full">
              <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
              <span className="text-xs font-semibold text-amber-700">{unreadCount} unread</span>
            </div>
          )}
        </div>
      </motion.div>
...
/* ═══════════════════════════
   CONTACT CARD
═══════════════════════════ */
function ContactCard({ contact: c, index, isExpanded, onToggle }) {
  const isEvent  = c.type === "event";
  const isHoreca = c.type === "horeca";

  /* ── per-type config ── */
  const typeConfig = {
    contact: { icon: <MessageSquare className="w-4 h-4" />, bg: "bg-blue-100 text-blue-600",   label: "Message",  preview: c.message?.slice(0, 80) || "—" },
    event:   { icon: <PartyPopper   className="w-4 h-4" />, bg: "bg-purple-100 text-purple-600",label: "Event",    preview: `${c.eventLocation || "—"} · ${c.gathering ? `${c.gathering} guests` : ""}` },
    horeca:  { icon: <UtensilsCrossed className="w-4 h-4"/>, bg: "bg-emerald-100 text-emerald-600",label: "HoReCa", preview: `${c.businessName || "—"} · ${c.businessType || ""} · ${c.city || ""}` },
  };
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {!c.isRead && <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" />}
            <span className={cn("text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full", cfg.bg)}>
              {cfg.label}
            </span>
          </div>
        ...
        <div className="hidden sm:flex flex-col items-end gap-1 shrink-0">
          ...
          <span className={cn(
            ...
            c.isRead ? "bg-green-50 text-green-600" : "bg-amber-50 text-amber-600"
          )}>
            {c.isRead ? <><CheckCircle className="w-2.5 h-2.5" />Read</> : <><Clock className="w-2.5 h-2.5" />Unread</>}
          </span>
        </div>

        {/* Chevron */}
        <div className={cn(
          "shrink-0 w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center transition-transform",
        ...
        {isExpanded && (
          ...
          >
            <div className="px-5 pb-5 pt-1 border-t border-gray-100 bg-gray-50/50">
              {c.type === "event"  ? <EventDetail  c={c} /> :
               c.type === "horeca" ? <HorecaDetail c={c} /> :
               <MessageDetail c={c} />}
            </div>
          </motion.div>
        )}
  ...
  return (
    <div className="pt-4 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <InfoChip icon={<Mail className="w-3.5 h-3.5" />} label="Email" value={
          <a href={`mailto:${c.email}`} className="text-[#2d5016] hover:underline">{c.email}</a>
        } />
        <InfoChip icon={<Phone className="w-3.5 h-3.5" />} label="Phone" value={
          <a href={`tel:${c.phone}`} className="hover:text-[#2d5016]">{c.phone}</a>
        } />
        <InfoChip icon={<Clock className="w-3.5 h-3.5" />} label="Received" value={
  ...
  return (
    <div className="pt-4 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <InfoChip icon={<Calendar className="w-3.5 h-3.5" />} label="Event Date" value={
          c.eventDate ? new Date(c.eventDate).toLocaleDateString("en-IN", { dateStyle: "long" }) : "—"
        } />
        <InfoChip icon={<MapPin className="w-3.5 h-3.5" />} label="Location" value={c.eventLocation || "—"} />
        <InfoChip icon={<Users className="w-3.5 h-3.5" />} label="Expected Guests" value={c.gathering ? `${c.gathering} people` : "—"} />
      </div>
      <InfoChip icon={<Clock className="w-3.5 h-3.5" />} label="Requested on" value={
        new Date(c.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })
      } />
      <a
        href={`tel:${c.phone}`}
...
function HorecaDetail({ c }) {
  return (
    <div className="pt-4 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <InfoChip icon={<Mail    className="w-3.5 h-3.5" />} label="Email"  value={
          <a href={`mailto:${c.email}`} className="text-[#2d5016] hover:underline break-all">{c.email}</a>
        } />
        <InfoChip icon={<Phone   className="w-3.5 h-3.5" />} label="Phone"  value={
          <a href={`tel:${c.phone}`} className="hover:text-[#2d5016]">{c.phone}</a>
        } />
        <InfoChip icon={<Building2      className="w-3.5 h-3.5" />} label="Business" value={c.businessName || "—"} />
        <InfoChip icon={<UtensilsCrossed className="w-3.5 h-3.5"/>} label="Type"     value={c.businessType || "—"} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <InfoChip icon={<MapPin  className="w-3.5 h-3.5" />} label="City"       value={c.city || "—"} />
        <InfoChip icon={<Clock   className="w-3.5 h-3.5" />} label="Received"   value={
          new Date(c.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })
        } />
      </div>
      <div className="bg-white rounded-xl border border-gray-100 p-4">
        ...
        <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{c.requirement || "—"}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <a href={`mailto:${c.email}?subject=Re: HoReCa Partnership Inquiry – Paanshala`}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#2d5016] hover:bg-[#3d6820] text-white text-sm font-semibold rounded-full transition-colors">
          <Mail  className="w-4 h-4" />Reply via Email
        </a>
        <a href={`tel:${c.phone}`}
