...
import {
  CheckCircle, Package, MapPin, Phone, Mail,
  ArrowRight, ShoppingBag, Download, Loader2,
  Clock, Truck, Home,
} from "lucide-react";
      ...
      <div className="relative overflow-hidden bg-linear-to-r from-[#1e3a0f] via-[#2d5016] to-[#1e3a0f] py-16 md:py-20">
        ...
        <div className="relative max-w-3xl mx-auto px-4 text-center">
          ...
          >
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-white mb-3" style={{ fontFamily: "Georgia, serif" }}>
              Thank You! 🎉
            </h1>
            <p className="text-white/65 text-base md:text-lg max-w-md mx-auto">
              Your order <span className="text-white font-bold">#{o.orderNumber}</span> has been placed successfully.
      ...
      <div className="max-w-3xl mx-auto px-4 py-10 space-y-6">
        ...
        >
          ...
          {isCOD && (
            <div className="mt-5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
              ...
              <span className="font-bold">₹{o.totalAmount}</span> ready at the time of delivery.
              {o.codCharges > 0 && (
        ...
        >
          <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-gray-400 font-mono">#{o.orderNumber}</span>
          </div>
          ...
          <div className="divide-y divide-gray-50">
            {o.items?.map((item, i) => (
              <div key={i} className="flex items-center gap-4 px-6 py-4">
                ...
                <p className="text-sm font-bold text-gray-900 shrink-0">₹{item.totalPrice}</p>
              </div>
            ))}
          </div>
        ...
        >
          ...
          {o.shippingAddress && (
            <div className="text-sm text-gray-600 space-y-1">
              ...
              <p>{o.shippingAddress.streetAddress}{o.shippingAddress.landmark && `, ${o.shippingAddress.landmark}`}</p>
              <p>{o.shippingAddress.city}, {o.shippingAddress.state} – {o.shippingAddress.pincode}</p>
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                <span className="flex items-center gap-1.5 text-xs text-gray-500">
                  <Phone className="w-3 h-3" />{o.shippingAddress.phone}
                </span>
                {o.shippingAddress.email && (
                  <span className="flex items-center gap-1.5 text-xs text-gray-500">
                    <Mail className="w-3 h-3" />{o.shippingAddress.email}
                  </span>
                )}
              </div>
            </div>
